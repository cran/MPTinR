...
From these findings the question arises of whether or not the restrictions on the guessing parameters are justified when taking both model fit and model flexibility into account. To this end we now also consider the information criteria and again only look at the summed individual results as well as the results of the aggregated data.\footnote{In previous versions of \pkg{MPTinR} the summed BIC and FIA values were reported incorrectly. Due to the non-linear nature of the $ln(N)$ terms in the BIC and FIA formulas (Equations 9 and 10) those values cannot be simply summed (which is what previous versions reported). Instead, the $G^2$ values, the $N$s, parameters, and the penalty factor for FIA needs to be summed first and the information criteria calculated from these summed values. From \pkg{MPTinR} version 1.9.2 onwards and both \texttt{fit.mpt} et al. and \texttt{select.mpt} report correctly summed BIC and FIA values.} Note that the FIA penalty term given in the output includes $ln(N)$ (i.e., it only excludes the first additive term in Equation 10, $1/2 \times G^2$).
...
As can be seen, this does not change the conclusions (i.e., the restricted model performs better). Furthermore, reporting of the aggregated data is suppressed now. Given that the data weas aggregated including all data sets (i.e., also the two now excluded ones), this makes a lot of sense as it avoids reporting of misleading results.
...
Overall the FIA results clearly favor the two models including the inequality restrictions on $g$. At the level of the individual data, the restricted 2HTM seems to be preferred given as its summed FIA value is clearly smaller by around 64 and additionally it provided the best account for more participants (23/22 versus 17/18 for the unrestricted 2HTM). On the level of the aggregated data the pattern is the same although the difference between restricted and unrestricted 2HTM is minimal (0.16). 
...
We can use this array to obtain mean or median estimates from the two models:
